...
<body style="background: #f2fff2;">
    ...
    <div class="container py-3">
        <h2 class="text-center "><b style="color: #00ca00;">WhatsApp CRM</b> App Users</h2>
        ...
        <div class="d-flex mt-5 mb-4 justify-content-between">
            ...
            <div class="d-flex">
                <div class="me-3">
                    ...
                    <option value="paid" style="background: #fff1d6;">Paid</option>
                    <option value="pending" style="background: #e8ffd6;">Pending</option>
                    </select>
                </div>
                <div>
        ...
        <table class="table table-hover" id="userTable" style="border: 1px solid rgb(204, 204, 204);">
            <thead>
                <tr style="height: 50px;">
                    ...
                    <th scope="col" class="t-head-bg">Due Date</th>
                    <th scope="col" class="t-head-bg">Payment</th>
                    <th scope="col" class="t-head-bg">No. of users</th>
            ...
            <tbody>
                ...
                <tr data-name="{{ $user->name }}" data-mobile="{{ $user->mobile }}" data-email="{{ $user->email }}"
                    ...
                    <td>{{ $user->mobile }}</td>
                    <td>{{ $user->email }}</td>
                    <td class="payment-date" data-payment-date="{{ $user->payment_date }}">
                        @if ($user->payment_date == 'pending')
                        Payment Pending
                        @else
                        {{ \Carbon\Carbon::parse($user->payment_date)->addYear()->format('d-m-Y') }}
                        @endif
                    </td>
                    <td>
                        @if ($user->payment_date == 'pending')
                        <span class="badge bg-pending" data-id="{{ $user->id }}">Pending</span>
                        @else
                        <span class="badge bg-paid" data-id="{{ $user->id }}">Paid</span>
                        @endif
                    </td>
                    <td>{{ $userGroups->firstWhere('app_id', $user->app_id)->total ?? 0 }}</td>
                    <td>
